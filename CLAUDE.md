...
- 协议：Google A2A v1.0 协议（兼容早期 JSON-RPC 版本）
- 前端：Next.js
- 插件 SDK：Python（`sdk/agentweb_sdk/`）
...
| 变量 | 必须 | 说明 | 不设置的后果 |
|------|------|------|-------------|
| `SITE_URL` | **是** | 平台对外访问地址（含协议和端口） | SDK 代码片段生成 `localhost` 地址，Agent 无法连接 |
...
### SDK 模式（推荐）
- 安装 `pip install agentweb-sdk`，配置持久 `agent_key`，一行代码启动
- Agent 通过 WebSocket 连接到 `/ws/agent?agent_key=xxx`，自动重连、心跳
- 首次连接无 Agent 记录时，平台自动触发自我介绍 → LLM 分类 → 创建 Agent
- 类似 IM 平台接入体验：配置一次，永久在线
- 代码示例：
  ```python
  from agentweb_sdk import AgentWebPlugin, HTTPAdapter
  plugin = AgentWebPlugin("http://platform-url", "ak_xxx")
  plugin.use_adapter(HTTPAdapter("http://your-agent:8000"))
  plugin.run()
  ```

### HTTP 端点 URL 模式
- 注册时填 `endpoint_url`（Agent 地址）、`endpoint_api_key`（可选认证密钥）和 `endpoint_protocol`（`openai` 或 `a2a`）
- 支持两种提供方协议：
  - **OpenAI 兼容**（默认）：Agent 提供 `/v1/chat/completions` 端点
  - **A2A 协议**：平台优先用 v1.0 REST 格式（`/message:send`、`/message:stream`），自动降级到早期 JSON-RPC 格式（`tasks/sendSubscribe`）
- 平台收到消费方请求后自动按 Agent 的 `endpoint_protocol` 翻译格式并转发
- 无需 WebSocket 连接、无需心跳，注册即上线
- 适用于已有 HTTP 服务的 Agent 框架

## 消费方调用协议

平台支持两种消费方调用协议，底层共享同一套路由和计量逻辑：

### OpenAI 兼容（`/v1/chat/completions`）
标准 OpenAI API 格式，适合直接替换 OpenAI endpoint 的场景。

### A2A 协议
Google A2A (Agent-to-Agent) 协议，适合 Agent 间互调。支持 v1.0 REST 格式和早期 JSON-RPC 格式。

#### v1.0 REST 格式（推荐）

```bash
# 非流式 POST /message:send
curl -X POST http://localhost:8000/api/v1/a2a/message:send \
  ...
  -d '{
    "message": {
      ...
      "role": "user",
      "parts": [{"text": "你好"}],
      "metadata": {"agent_id": "AGENT_UUID"}
    }
  }'

# 流式 POST /message:stream（SSE）
curl -X POST http://localhost:8000/api/v1/a2a/message:stream/AGENT_UUID \
  -H "Authorization: Bearer sk-xxx" \
  -H "Content-Type: application/json" \
  -d '{
    "message": {
...
curl -X POST http://localhost:8000/api/v1/a2a/rpc \
  -H "Authorization: Bearer sk-xxx" \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":"1","method":"tasks/send","params":{"taskId":"t1","message":{"role":"user","parts":[{"kind":"text","text":"你好"}],"metadata":{"agent_id":"AGENT_UUID"}}}}'

# 流式 tasks/sendSubscribe（SSE）
curl -X POST http://localhost:8000/api/v1/a2a/rpc \
  -H "Authorization: Bearer sk-xxx" \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":"1","method":"tasks/sendSubscribe","params":{"taskId":"t1","message":{"role":"user","parts":[{"kind":"text","text":"你好"}],"metadata":{"agent_id":"AGENT_UUID"}}}}'
```
